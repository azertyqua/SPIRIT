...
exports.run = (bot, msg, args) => {
    if (args.length < 1) {
        ...
        if (shortcuts.length < 1) {
            throw 'Vous n\'avez aucun raccourci!';
        }

        const list = shortcuts.map(shortcut => `- \`${shortcut.name}\``).join('\n');

        msg.delete();
        return bot.utils.sendLarge(msg.channel, `**Raccourcis:**\n${list}`, { cutOn: '\n' });
    }
    ...
    if (ADD.test(args[0]) || EDIT.test(args[0])) {
        ...
        if (!shortcut) {
            throw `Le raccourci \`${id}\` n'existe pas !`;
        }
...
exports.info = {
    ...
    description: 'Contrôle ou répertorie vos raccourcis',
    examples: [
