...
exports.run = async (bot, msg, args) => {
    if (args.length < 1) {
        throw 'Vous devez spécifier un référentiel ou un terme de recherche!';
    }
    ...
    if (input.indexOf('/') !== -1) {
        ...
        msg.edit(`:arrows_counterclockwise: Chargement des informations pour '${repo}'...`);
        ...
        if (json.total_count < 1) {
            throw `Aucun résultat trouvé pour '${args.join(' ')}'`;
        }
...
exports.info = {
    ...
    description: 'Liens vers un référentiel GitHub'
};
